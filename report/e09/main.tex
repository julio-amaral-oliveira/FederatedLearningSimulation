...
%%% 
\title{Uncertainty-driven Quorum-based Reactive Retraining in FL under Gradual Concept Drift}
%%% Proposta para tentar reduzir o tamanho do título.
...
\author{
\IEEEauthorblockN{Authors anonymized for the revieweing process}
}
%\IEEEauthorblockA{\IEEEauthorrefmark{1}Institute of Computing, State University of Campinas (UNICAMP), Brazil\\
%j230537@dac.unicamp.br, bit@unicamp.br}
%\IEEEauthorblockA{\IEEEauthorrefmark{2}Federal University of Rio de Janeiro (UFRJ), Brazil\\
...
\begin{abstract}
%Federated Learning (FL) models deployed in real-world environments face concept drift, which refers to changes in the data distribution after model deployment, reducing the model’s accuracy in service until retraining. In real-world scenarios, the change is not always abrupt; the proportion of degraded inputs increases over a transition interval. This work proposes ReacQD (Reactive Quorum Detection) to trigger reactive retraining under gradual drift. Each client monitors a sequence of unlabeled images using a local uncertainty-based detector, which sends a signal when it identifies a change in the distribution. The server tracks the signals at each monitoring step and keeps a record, over recent steps, of the distinct clients that have signaled a change. Retraining using the data observed at the time of triggering is only triggered when the fraction of these clients reaches a minimum quorum, even if the signals come from different steps. The rule requires confirmation from distinct clients, so that an isolated local false positive is insufficient to trigger retraining, and it accumulates signals spread out over time, which is of paramount importance when different clients experience drift at different times (staggered drift). The evaluation uses CIFAR-10 images under four visual degradations, with five seeds and three transition durations (50--200\,s), totaling 60 episodes that compare ReacQD with a baseline model without retraining. The metrics are service downtime, calculated based on the cumulative time during which the accuracy of the model in service remains below a quality threshold, and detection delay. ReacQD detects drift in 59 out of 60 episodes and reduces the average downtime from 408--562\,s to 0--178\,s, eliminating it in the best-case scenario; long transition intervals also better preserve accuracy on the undegraded set. No false-positive trigger is observed in the 15 control episodes without drift (0/15), and the collective criterion triggers in all 20 evaluated episodes of staggered drift, while Gaussian noise is the most challenging scenario. The work also contributes a service downtime metric, a federated uncertainty detector, and a verifiable experimental infrastructure. In the evaluated regime, the unlabeled ReacQD is effective, and detection delay depends on the scenario.

Federated Learning (FL) models deployed in real-world environments face concept drift, which refers to shifts in data distribution that degrade service accuracy over time. These shifts are often gradual, with the proportion of drifted inputs increasing progressively rather than changing abruptly. This work proposes ReacQD (Reactive Quorum Detection), which reactively triggers retraining under gradual drift. Each client monitors unlabeled images with a local uncertainty-based detector and signals the FL server upon detecting drift. The server tracks distinct clients signaling drift within a sliding window and triggers retraining, using data from the signaling period, only once a minimum quorum of clients is reached, even if signals arrive at different steps. This quorum-based approach filters out isolated false positives while accumulating evidence over time, which is critical for staggered drift, where clients experience drift at different times. We evaluate ReacQD on CIFAR-10 under four visual degradations, with five seeds and three transition durations (50–200$\;$s), totaling 60 episodes compared against a no-retraining baseline. We measure service downtime (cumulative time below an accuracy threshold) and detection delay. ReacQD detects drift in 59 of 60 episodes, reducing average downtime from 408–562$\;$s to 0–178$\;$s (eliminating it in the best case); longer transitions also better preserve accuracy on undegraded data. No false positives occur across 15 no-drift control episodes, and the quorum criterion successfully triggers in all 20 staggered-drift episodes, with Gaussian noise as the most challenging scenario. This work also contributes a service downtime metric, a federated uncertainty detector, and a reproducible experimental infrastructure. Overall, ReacQD is effective using only unlabeled data, though detection delay varies by scenario.
\end{abstract}

\begin{IEEEkeywords}
federated learning, concept drift, drift detection, reactive retraining, collective quorum-based decision-making, unsupervised detection, service downtime
\end{IEEEkeywords}

\section{Introduction}\label{sec:intro}

Federated Learning (FL) trains a shared model using data distributed among clients to preserve privacy~\cite{mcmahan2017}. Once deployed, client data distributions may drift, degrading the in-service model's quality until retraining is performed. Detecting such changes, generally unlabeled at inference time, is a prerequisite for triggering retraining. Drift is not always abrupt: under gradual drift, new and old data coexist, and the proportion of shifted inputs grows slowly over a transition interval, as defined in the non-federated drift-detection literature~\cite{poenaru2022,cerqueira2026}.

We adopt the architecture prevalent in the FL drift literature, combining local detection with central decision-making~\cite{jothimurugesan2023,chow2023,mavromatis2024,patel2026,han2026,fu2025,zhou2026,casado2022,chen2026}. Local detection is the known response to the failure of server-side aggregated tests under staggered drift across clients~\cite{jothimurugesan2023}. Prior work uses such signals for client-side mitigation~\cite{chow2023,mavromatis2024}, client regrouping~\cite{fu2025,zhou2026}, or selection of updates for aggregation~\cite{patel2026}. The closest comparable approach, FL-MalDrift, uses a drift threshold to filter which updates are incorporated into the global model, but does not specify when retraining should be triggered~\cite{patel2026}. This work's main contribution lies precisely in the rule that converts a local signal into a retraining trigger.

This paper proposes ReacQD, in which the server triggers retraining only when a minimum fraction of distinct clients signals drift within a sliding window of recent steps. This (i) prevents an isolated local false positive under continuous monitoring~\cite{poenaru2022} from alone triggering retraining, and (ii) accumulates signals from clients drifting at different times. Each client runs Uncertainty-based Drift Detection (UDD)~\cite{baier2023} on the shared global model, rather than in the original centralized setting. Monte Carlo (MC) dropout~\cite{gal2016} keeps dropout layers active during inference, processing the same batch $T$ times with a distinct random mask per run and without updating weights. The average Shannon entropy of the resulting stochastic predictions summarizes uncertainty, monitored over time by an adaptive windowing test (ADWIN)~\cite{bifet2007} to signal change. Once triggered, retraining consists of a single global round over the data each client has observed at that time, in line with the explicit retraining limits of DriftGuard~\cite{han2026} and ShiftEx~\cite{bhope2025}, and with Modyn's separation of triggering from data selection~\cite{bother2025}. We model gradual concept drift as a linear increase in the fraction of degraded data, from zero to one, over a configurable number of monitoring steps~\cite{cerqueira2026}.
%
We evaluate the proposal using service downtime as the primary metric: the cumulative time during which the in-service model's accuracy remains below a quality threshold $\tau$ under degradation. Experiments cover both short transitions (the abrupt regime) and a long transition, in which the detector operates mid-change. None of the reviewed works on drift in FL quantify this downtime. The main contributions of this work are thus:

\begin{itemize}
    \item ReacQD, which aggregates unlabeled per-client signals within a window of steps and requires confirmation from distinct clients before triggering retraining;

    \item a characterization of detection delay through sensitivity analyses on quorum and window size, and experiments under staggered drift;

    \item a service downtime metric for FL under drift, accumulating the time during which in-service accuracy remains below a quality threshold $\tau$;

    \item a federated implementation of UDD, with one detector per client sharing the global model, extending the centralized detector of Baier et al.~\cite{baier2023};

    \item a verifiable experimental infrastructure pairing ReacQD with a no-retraining baseline, using the same saved model, random state, data sequence, and horizon; and
...
\medskip\noindent
\textbf{Quality threshold and downtime.} Let $a(t)$ denote the in-service model accuracy, evaluated on a test set that reflects the current drifted fraction $f(t)$. The quality threshold $\tau$ defines the service as unavailable whenever $a(t) < \tau$. Since accuracy is evaluated only at discrete times, downtime ($D$) over $[t_0, t_0+H]$ is defined as the sum of the durations between consecutive evaluations for which the accuracy measured at the start of the interval falls below $\tau$. Formally,
...
\label{eq:downtime}
\end{equation}
where $t_1 = t_0, \dots, t_m$ are the evaluation times and $t_{m+1} = t_0+H$. Between evaluations, accuracy is assumed to hold its last measured value, both within each interval and, in the final interval, until the horizon $t_0+H$. Evaluations occur at three points: the boundaries of each monitoring step (every $\Delta t$ seconds); the moment retraining ends (an additional evaluation capturing recovery immediately after the round, even if this falls between two step boundaries); and the horizon $t_0+H$.

\section{Proposed Method: ReacQD}
ReacQD combines initial training on undegraded data, local monitoring of unlabeled data, and a collective quorum-based decision to trigger retraining when the signals reach the minimum quorum. Algorithm~\ref{alg:reactive} summarizes the procedure, and the parameters are given in Section~\ref{sec:setup}.


\begin{algorithm}[!bh]
\footnotesize
\caption{ReacQD: reactive retraining with local uncertainty detection and collective quorum-based decision-making.}
\label{alg:reactive}
\begin{algorithmic}[1]
...
\subsection{Collective Decision by Quorum} At each operational step, the server receives signals from each client. Let $S_s$ be the set of clients that send a signal at step $s$; the server maintains the union of the signals over the window of the last $W$ steps, $\hat{S}_s = \bigcup_{i=s-W+1}^{s} S_i$, and triggers retraining at step $s_d$ when $|\hat{S}_{s_d}|/C \ge \theta$; the trigger time is $t_d=t_0+s_d\,\Delta t$. Two parameters define the decision rule: the quorum $\theta$, which controls how many confirmations is required to take action, and the window $W$, which controls how long signals from distinct clients can accumulate. Thus, an isolated local false positive under continuous monitoring~\cite{poenaru2022} is insufficient to trigger retraining. Once triggered, the server does not make another decision during that episode, so each episode has at most one retraining.

\subsection{Retraining with Trigger Data} After triggering, ReacQD performs retraining limited to $K$ synchronous rounds of FL with all clients using the data available at the time of triggering.
...
\end{table*}
\section{Results}
We present group averages across the five seeds and evaluate two hypotheses: that ReacQD reduces downtime compared with the baseline without retraining, and that detection delay increases with the duration of the transition. The primary measures are detection delay, the transition fraction at triggering ($f(t_d)$, the fraction of the transition elapsed at triggering), and downtime. The secondary metrics are the change in accuracy on the undegraded set (final accuracy on that set minus the accuracy on that set before drift) and the final accuracy under degradation.

\subsection{Detection: 59 Out of 60 Episodes}
Detection was successful in 59 out of 60 episodes (14 out of 15 in the Gaussian noise scenario); the only failure occurred in the Gaussian noise scenario, during the 100\,s transition interval, with seed 42. The delay increases with the duration of the transition for the three visual degradations (Table~\ref{tab:delay}), consistent with the ADWIN analysis for gradual change with constant slope~\cite{bifet2007}. The Gaussian noise does not exhibit the same trend.

\begin{table}[!t]
\centering
\caption{Detection delay (s) and the transition fraction at triggering (averages per group over 5 seeds; detection rate per group; the Gaussian seed that failed at 100\,s is excluded from that group’s delay average).}
\label{tab:delay}
\small
...
\subsection{Downtime: ReacQD versus Baseline}
\begin{table}[!t]
\centering
\caption{Downtime (s): ReacQD versus paired baseline without retraining (group averages over 5 seeds; format: reactive / baseline).}
\label{tab:downtime}
\small
...
\end{table}
ReacQD reduces the average downtime across the 12 groups (Table~\ref{tab:downtime}) from 408--562\,s to 0--178\,s, and exhibits lower downtime than the baseline in all triggered episodes (59 out of 60). The fog scenario with a transition interval of 200\,s achieves \textbf{zero downtime} at $\tau=0.50$ (Fig.~\ref{fig:fog}). Triggering occurs when the transition fraction reaches 0.69, at which point the accuracy is still 0.57 (above $\tau$). The single round maintains accuracy above $\tau$ (final accuracy under degradation of 0.71).

The ordering between ReacQD and the baseline remains consistent as the quality threshold varies. For $\tau \in \{0.40, 0.45, 0.50, 0.55\}$, ReacQD’s downtime does not exceed that of the paired baseline in any of the 48 cells, although the magnitudes vary with $\tau$ (the average contrast decreases by about 47\% with $\tau=0.40$; the worst-case scenario for ReacQD increases to 190\,s with $\tau=0.55$).
...
\textbf{Staggered drift.} When clients experience drift in different steps (client $i$ experiences drift in operational step $s=i$, without a gradual transition; 20 episodes), the collective triggering criterion triggers in 20 out of 20 episodes at the quorum threshold (signaled fraction 0.30--0.40). ReacQD prevents approximately 81\% of the baseline’s downtime (600\,s, permanent drift) in three of the four degradation scenarios, with 116\,s of downtime in ReacQD, and 70\% in the Gaussian noise scenario, with 180\,s; since the drift is instantaneous and permanent, the delay translates into downtime in the same proportion (116\,s corresponds to 108\,s of delay plus 8\,s for the retraining round).

\subsection{False Positive Rate in Sequences Without Drift}
Fifteen control episodes (five seeds and three transition durations) were run on undegraded data. Each client monitors an undegraded sequence for the full 600\,s horizon. The collective trigger criterion does not trigger in any episode. None of the 15 episodes results in a retraining decision, and the local detectors send no signals across the 9,000 client-step evaluations. Eight transient entropy variations (values above 1.0) occur, but ADWIN does not signal any of them since the test evaluates changes in the distribution, not transient variations. The criterion, therefore, does not trigger in sequences without drift; no false-positive trigger is observed in the 15 control episodes (0/15), and it triggers under drift in 59 of 60 episodes.

\subsection{Accuracy Variation in the Undegraded Set and Recovery Behavior}
\begin{table}[!t]
\centering
\caption{Accuracy variation in the undegraded set $\Delta$ (ReacQD; group mean $\pm$ standard deviation across 5 seeds).}
\label{tab:retention}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccc}
\toprule
...
(i) Long transition intervals partially preserve accuracy in the undegraded set (Table~\ref{tab:retention}). With a transition interval of 200\,s, the retraining set retains about 30\% of the undegraded data, and the loss of accuracy in this set is reduced to 1--2 percentage points (compared to 9--21 for short transition intervals) for fog, frosted-glass, and motion blur; Gaussian noise does not show this improvement (transition fraction of 0.97 at triggering). (ii) Recovery persists until the horizon, as no episode returns to values below $\tau$ within the horizon; the retrained model retains accuracy on the evaluated fully degraded regime. In 9 of the 20 episodes with a transition interval of 200\,s (all five fog episodes, three frosted-glass episodes, and one motion blur episode), the accuracy remains above $\tau$. The average final accuracy per group, in the fully degraded regime, ranges from 0.60 to 0.71 for ReacQD (except in the failed episode, where the final accuracy is 0.33) and from 0.35 to 0.48 for the baseline (Fig.~\ref{fig:motion}).

\begin{figure}[!t]
\centering
\includegraphics[width=\linewidth]{figures/fig_motion_ramp20_traj.pdf}
\caption{Motion blur, transition interval of 200\,s, five paired seeds. ReacQD recovers within one round and remains above $\tau$ for most of the time horizon; the baseline continues to decline to about 0.35.}
\label{fig:motion}
\end{figure}

\section{Discussion}
Monitoring incurs computational costs. Over the course of the 80-step episode (60 operational steps plus 20 detector warm-up steps), the detector with $T=5$ costs approximately 6.8\,TFLOP ($6.8\times10^{12}$ floating-point operations; 4\,000 model runs, each on a batch of 32 images, with 50 runs per step across the ten clients) compared to about 8.0\,TFLOP for the single retraining round. The ratio ranges from 0.85 to 0.96 depending on the accounting method; that is, monitoring accounts for about 47\% of reactive computation. On the ten clients, monitoring per step takes about 0.13\,s, or 1.3\% of each 10-s step; on the device used for measurement, a detector update takes about 12.7\,ms per client. Maintaining the published UDD configuration with $T=50$ would increase the monitoring cost to about nine times that of the retraining round, which is why adopting $T=5$ was a cost-driven choice.

The conclusions regarding downtime remain valid even for a round ten times longer. Recalculating the downtime from the recorded time series, assigning the pre-retraining accuracy to the interval $[t_d,t_d+d]$, where $d$ is the round duration, using the first post-retraining record onwards, and applying the limit rule shows that an 80\,s round still reduces the average downtime from 507.5\,s (baseline) to 122.2\,s (76\%). The group ordering, the case of zero downtime in the fog, and the behavior limited by detection in Gaussian noise remain unchanged; the extra cost is exactly the pre-retraining interval plus the longer round. With $d=800$\,s, the round no longer fits within the 600\,s operating horizon, and the limit rule would discard retraining in the 59 episodes in which triggering occurred, so that ReacQD would become equivalent to the baseline due to the limit rule, rather than due to training dynamics; the limit is the smallest difference between the trigger time and the horizon, approximately 140\,s in this configuration.

\subsection{Gaussian Noise and Detector Sensitivity}
Gaussian noise is the only scenario with incomplete detection. With a transition interval of 100\,s and seed 42, ReacQD does not trigger, since only four of the ten clients send a signal throughout the entire episode, and never three within the same two-step window (maximum signaled fraction of 0.20 in sparse steps); the other four seeds detect the drift at 120--210\,s. Since the window and quorum are fixed across seeds, the failure is best explained by a loss of sensitivity in the local detectors. Without triggering, the proposed method is exactly equivalent to the baseline (downtime of 540\,s). With a transition interval of 200\,s, the delays range from 180 to 460\,s (standard deviation of 104\,s), but ReacQD still reduces downtime by 75\% compared to the baseline (112 vs.\ 448\,s), in line with the risk that ADWIN-type detectors face under long drifts with smooth transitions~\cite{poenaru2022}. Gaussian noise is also the scenario with the longest detection delay. Fig.~\ref{fig:gaussian} shows the seed that failed.
...
This work proposes and characterizes ReacQD, a quorum-based collective decision policy for reactive retraining in synchronous federated learning under gradual drift. Each client monitors its unlabeled sequence, and the server triggers retraining only when confirmation from distinct clients reaches the quorum within a window of steps. ReacQD was evaluated against a paired baseline without retraining across 60 episodes, using service downtime as the primary metric. Detection was successful in 59 out of 60 episodes, and, for visual degradations, the delay increases with the duration of the transition; ReacQD reduced the average downtime from 408--562\,s to 0--178\,s (zero in the case of fog with a transition interval of 200\,s); and long transition intervals preserved accuracy on the undegraded set for visual degradations. The collective triggering criterion triggered in all 20 episodes of staggered drift (70--81\% of the baseline downtime avoided) with no false-positive trigger observed in 15 control episodes without drift (0/15). Future work includes (i) periodic triggering by schedule, (ii) oracle triggering with knowledge of the drift instant, and (iii) a server-based detector as a baseline; more seeds and formal analysis of dispersion; recurrent drift; non-IID partitions; incremental drift; larger models; sensitivity to the parameters $T$, $\delta$, and detector calibration limits.

\section*{Availability of Code and Data}
The implementation and all materials (the 60 paired episodes, comparison variants, aggregated summaries, and seed-specific tables) are available in a github repository that will be disclosed after acceptance to preserve the double-blind review process.%at \url{https://github.com/julio-amaral-oliveira/FederatedLearningSimulation}.

\section*{Acknowledgments}
This section will be populated after acceptance to preserve the double-blind review process.
