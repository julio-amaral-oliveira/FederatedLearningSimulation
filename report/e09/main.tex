...
\begin{itemize}
    ...
    \item characterization of detection latency through sensitivity analyses regarding quorum and window size, as well as experiments under stepwise drift;
...
\label{alg:reactive}
\begin{algorithmic}[1]
\State \textbf{Input:} $C$ clients; $R$ initial rounds on undegraded data; $T$ stochastic model runs with MC dropout; sensitivity $\delta$; quorum $\theta$; window $W$; retraining threshold $K$; training set size $n_{\mathrm{tr}}$; test set size $n_{\mathrm{te}}$
...
\subsection{Downtime: ReaQI versus reference}
\begin{table}[!t]
\centering
\caption{Downtime (s): ReaQI versus paired reference without retraining (group averages over 5 seeds; format: reactive / reference).}
\label{tab:downtime}
\small
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccc}
