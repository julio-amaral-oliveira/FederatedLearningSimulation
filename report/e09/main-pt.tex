...
Nossas contribuições são: (i) uma \textbf{política de decisão coletiva por quórum} que agrega alarmes sem rótulos por cliente em uma janela de passos e exige a corroboração de clientes \emph{distintos} antes de disparar o retreinamento; a política está ausente na literatura revisada e é a mais próxima da barreira de participação do FL-MalDrift \cite{patel2026}; caracterizamos sua latência de detecção por meio de análises de sensibilidade a quórum e janela, além de experimentos sob drift staggered; (ii) uma \textbf{métrica de downtime de serviço} para FL sob drift, que acumula o tempo em que a acurácia do modelo em serviço permanece abaixo de um limiar de qualidade $\tau$; (iii) uma \textbf{implementação federada do UDD}, com um detector por cliente compartilhando o modelo global, estendendo o detector centralizado de Baier et al.~\cite{baier2023}; (iv) uma \textbf{infraestrutura experimental auditável} que pareia a variante reativa com uma baseline sem retreinamento a partir do mesmo checkpoint, estado aleatório, fluxo e horizonte; e (v) uma \textbf{caracterização empírica do drift gradual} ao longo de 60 episódios pareados.
...
Um achado recorrente é que a detecção de drift precisa ser local. Jothimurugesan et al.~\cite{jothimurugesan2023} mostram que testar o erro agregado no servidor falha durante transições staggered, porque os erros de clientes que já sofreram drift e dos que ainda não sofreram se cancelam. FLARE \cite{chow2023}, FLAME \cite{mavromatis2024}, FL-MalDrift \cite{patel2026}, DriftGuard \cite{han2026}, FedDAA \cite{fu2025}, FedPLC \cite{zhou2026}, FELK/FL-HVD \cite{chen2026} e Casado et al.~\cite{casado2022} seguem detecção local com decisão central. Em outros trabalhos: o FLASH detecta drift no servidor a partir da magnitude das atualizações agregadas de parâmetros \cite{panchal2023}; Stallmann et al.\ usam um detector global por agrupamento fuzzy \cite{stallmann2024}; e o Adaptive-FedAVG dispensa por completo a detecção, adaptando a taxa de aprendizado no servidor para aumentar a plasticidade da fase de aprendizado \cite{canonaco2021}.

Adotamos a arquitetura predominante porque a detecção local é a resposta da literatura à falha dos testes agregados no servidor; a contribuição deste trabalho não está no sinal local, mas na regra que o transforma em decisão. Nesse ponto, o que se faz com o alarme varia entre os trabalhos revisados e, em geral, exige menos corroboração: FLARE e FLAME acionam a mitigação a partir do próprio endpoint \cite{chow2023,mavromatis2024}; FedDAA e FedPLC reorganizam os clientes por agrupamento \cite{fu2025,zhou2026}; o DriftGuard formaliza no servidor a tupla gatilho/participação/parâmetros \cite{han2026}; e o FL-MalDrift usa uma barreira de participação que regula quais clientes agregam, não quando disparar \cite{patel2026}. Em nenhum deles a decisão exige a corroboração de múltiplos clientes \emph{distintos} ao longo de uma janela temporal. No nosso trabalho, o disparo requer um quórum de clientes distintos sinalizados, possivelmente em passos diferentes da janela, o que (i) evita que um falso positivo local isolado sob monitoramento contínuo \cite{poenaru2022} acione o retreinamento e (ii) acumula alarmes de clientes que sofrem drift em momentos distintos---situação em que um gatilho de passo único falharia.

\subsection{Detecção de drift sem rótulos}
O UDD de Baier et al.~\cite{baier2023} é o método de referência sem rótulos; a Seção IV-A descreve nossa implementação. O ADWIN é preferido a DDM/EDDM porque estes exigem entradas binomiais, enquanto o sinal de entropia não se restringe a esse tipo \cite{baier2023}. Lobo et al.~\cite{lobo2023} conectam concept drift e incerteza do modelo, e sabe-se que estimadores de incerteza perdem confiabilidade sob dataset shift \cite{ovadia2019}. O D3M \cite{nguyen2025} é um método complementar sem rótulos, baseado em discordância entre modelos, com garantias quanto a falsos positivos sob o pressuposto de covariate shift. O PUDD \cite{lu2025} é igualmente complementar, pois aplica um teste qui-quadrado de Pearson a um índice de incerteza de predição, cujo nível de significância controla a taxa de falsos positivos --- ao contrário da etapa ADWIN do UDD, que acompanha a média.
...
A política reativa combina treino limpo, monitoramento local sem rótulos e uma decisão coletiva por quórum, que dispara o retreinamento quando os alarmes atingem o quórum mínimo. O Algoritmo~\ref{alg:reactive} resume o fluxo; os parâmetros estão na Seção~\ref{sec:setup}.
