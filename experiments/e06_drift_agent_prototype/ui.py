...
Uso:
    streamlit run experiments/e06_drift_agent_prototype/ui.py
"""
...
def main():
    ...
    st.markdown(
        "Visualização dos resultados legados do fluxo "
        "`smoke_drift` (agora `experiments/e07_drift_agent/episode.py`)."
    )
    ...
    if not json_files:
        st.sidebar.warning(
            ...
            "Os resultados legados do E06 usam o formato antigo. As novas "
            "execuções do E07 publicam pares v3 em `results/e07-drift-agent/`."
        )
        st.stop()
